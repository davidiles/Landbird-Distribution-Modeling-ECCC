...
ARU_summary <- ARU_sf %>%
  ...
  select(organization,project,min_year,max_year,n_surveys)
...
ARU_summary <- ARU_summary %>%
  subset(organization %!in% c("ABMI","BU","JNP","TTEC","PC","BNP","ACE"))
# ---------------------------------------------------------
...
ARU_recordings_SPT[duplicated(ARU_recordings_SPT$recording_id),]$recording_id
...
mean(ARU_counts_SPT$recording_id == ARU_recordings_SPT$recording_id) 
