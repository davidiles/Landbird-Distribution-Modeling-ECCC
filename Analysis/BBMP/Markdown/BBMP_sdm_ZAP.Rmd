...
for (sp_code in (names(species_to_model))[1:50]){
  for (xval_fold in (1:5)){
    ...
    model_components <- as.formula(paste0('~ Intercept_presence(1) +spde_presence(main = geometry, model = matern_coarse) +',paste0("Beta_presence_",covariates_to_include,'(',covariates_to_include,',model="linear", mean.linear = 0, prec.linear = ', prec_linear[1],')', collapse = " + "),'+','Intercept_count(1) + spde_count(main = geometry, model = matern_coarse) +',paste0("Beta_count_",covariates_to_include,'(',covariates_to_include,',model="linear", mean.linear = 0, prec.linear = ', prec_linear[1],')', collapse = " + ")))
      ...
      if (package_version(getNamespaceVersion("INLA")) < "2023.10.19-1") {
        like(
          ...
          formula = as.formula(paste0('count ~ Intercept_count + spde_count + ',
                                      paste0("Beta_count_",covariates_to_include, collapse = " + "))),
        ...
        tryCatch(expr = {bru(components = model_components,
                             ...
                             like(family = "binomial",
                                  formula = as.formula(paste0('presence ~ Intercept_presence + spde_presence + ',
                                                              paste0("Beta_presence_",covariates_to_include, collapse = " + "))),
    ...
    pred_without_BBMP <- predict(
      ...
      ~ {
        ...
        lambda <- exp(Intercept_count + spde_count + Beta_count_PC1 + Beta_count_PC2 + Beta_count_PC3 + Beta_count_PC4 + Beta_count_PC5 + Beta_count_PC6 + Beta_count_PC7 + Beta_count_PC8 + Beta_count_PC9)
