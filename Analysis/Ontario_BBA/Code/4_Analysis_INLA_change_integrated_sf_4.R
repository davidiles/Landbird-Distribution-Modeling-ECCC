...
square_centroids <- st_centroid(squares)

# ----------------------------------------------------
# Create a grid of hexagons across the province to summarize goodness of fit / data availability
# ----------------------------------------------------
...
species_to_model <- subset(species_to_model, n_detections >= 500) %>% unique()
...
for (spec in species_list){
  ...
  for (etype in c("nbinomial")){
    ...
    print(paste0(species_name,"-",etype))
    if (file.exists(paste0("../Output/Prediction_Maps/Relative_Abundance/",species_name,"_",etype,".png"))) next
    ...
    # ----------------------------------------------------
    # Summarize observed data in each atlas square
    # ----------------------------------------------------
    ...
    species_square_summary <- species_square %>%
      as.data.frame() %>%
    ...
    # ----------------------------------------------------
    # Only fit model if species was detected in at least 20 squares in both atlases
    # ----------------------------------------------------
    
    n_square_OBBA2 <- subset(species_square_summary,Atlas=="OBBA2" & count > 0)
    n_square_OBBA3 <- subset(species_square_summary,Atlas=="OBBA3" & count > 0)
    ...
    # Note: this also simulates entirely new square-level random effects, as a stronger test of model adequacy
    # ----------------------------------------------------------------------------
    ...
    if (etype == "nbinomial"){
      size_OBBA2 <- fit_INLA$summary.hyperpar[1,1]
      size_OBBA3 <- fit_INLA$summary.hyperpar[2,1]
      for (samp in 1:n_samp){
        
        sim_obs_OBBA2[,samp] <- rnbinom(nrow(sim_obs_OBBA2), size = size_OBBA2, mu = exp(fit_obs_OBBA2[,samp]))
        sim_obs_OBBA3[,samp] <- rnbinom(nrow(sim_obs_OBBA3), size = size_OBBA3, mu = exp(fit_obs_OBBA3[,samp]))
      }
    }
    ...
    Dispersion_OBBA2 <- testDispersion(DHARMaRes_OBBA2)
    ZeroInflation_OBBA2 <- testZeroInflation(DHARMaRes_OBBA2)
    ...
    Dispersion_OBBA3 <- testDispersion(DHARMaRes_OBBA3)
    ZeroInflation_OBBA3 <- testZeroInflation(DHARMaRes_OBBA3)
    
    # extract scaled residuals (for plotting)
    resid_OBBA2 <- DHARMaRes_OBBA2$scaledResiduals
    resid_OBBA3 <- DHARMaRes_OBBA3$scaledResiduals
    
    # Fill in predictions in dataframe
    species_square$pred_mean <- NA
    species_square$pred_mean[species_square$Atlas == "OBBA2"] <- apply(sim_obs_OBBA2,1,mean)
    species_square$pred_mean[species_square$Atlas == "OBBA3"] <- apply(sim_obs_OBBA3,1,mean)
    
    # Fill in residuals in dataframe
    species_square$resid <- NA
    species_square$resid[species_square$Atlas == "OBBA2"] <- resid_OBBA2
    species_square$resid[species_square$Atlas == "OBBA3"] <- resid_OBBA3
    
    end2 <- Sys.time()
    
    # ----------------------------------------------------------------------------
    # Plot raw data for each species in 25 km hexagons (hexagons just used because they are visually appealing)
    # ----------------------------------------------------------------------------
    
    # Intersect with survey information
    species_hex <- species_square %>% st_intersection(hexgrid)
    species_hex$Atlas <- "OBBA2"
    species_hex$Atlas[year(species_hex$Date_Time) >= 2020] <- "OBBA3"
    
    # Summarize total effort and mean count in each hexagon
    species_hex_summary <- species_hex %>%
      as.data.frame() %>%
      group_by(hexid,Atlas) %>%
      summarize(count = sum(count),
                pred_mean = sum(pred_mean),
                effort = sum(Survey_Duration_Minutes),
                n = n(),
                resid_mean = mean(resid)) %>%
      mutate(count_per_effort = count/effort,
             pred_per_effort = pred_mean/effort)
    
    species_hex_summary <- full_join(hex_centroids %>% select(hexid),species_hex_summary) %>% na.omit()
    
    col_lim <- c(min(species_hex_summary$count_per_effort[species_hex_summary$count_per_effort>0],na.rm = TRUE),max(species_hex_summary$count_per_effort,na.rm = TRUE))
    ...
    species_data_OBBA2 <- ggplot() +
      ...
      geom_sf(data=subset(species_hex_summary,count==0 & Atlas == "OBBA2"), aes(size=effort), shape = 1, col = "gray80")+
      geom_sf(data=subset(species_hex_summary,count>0  & Atlas == "OBBA2"), aes(col = count_per_effort, size = n))+
      ...
      scale_size_continuous(name = "# surveys", range = c(1,4), guide = "none")
    
    # species_data_OBBA2
    
    species_data_OBBA3 <- ggplot() +
      geom_sf(data=ONBoundary,colour="gray90", fill = "white")+
      geom_sf(data=subset(species_hex_summary,count==0 & Atlas == "OBBA3"), aes(size=effort), shape = 1, col = "gray80")+
      geom_sf(data=subset(species_hex_summary,count>0  & Atlas == "OBBA3"), aes(col = count_per_effort, size = n))+
      geom_sf(data = range %>% st_intersection(ONBoundary), col = "gray50", fill = "transparent", linetype = 2)+
      geom_sf(data = ONBoundary,colour="black",fill=NA,lwd=0.3,show.legend = F) +
    ...
    # ----------------------------------------------------------------------------
    # Plot mean residual values in each hexagon
    # ----------------------------------------------------------------------------
    
    col_lim <- c(0,1)
    colscale_resid <- RColorBrewer::brewer.pal(11,"PuOr") #colorRampPalette(c("red","white","blue"))(11)
    
    species_resid_OBBA2 <- ggplot() +
      geom_sf(data=ONBoundary,colour="gray90", fill = "white")+
      geom_sf(data=subset(species_hex_summary,Atlas == "OBBA2" & n>=10), aes(col = resid_mean, size = n))+
      geom_sf(data = range %>% st_intersection(ONBoundary), col = "gray50", fill = "transparent", linetype = 2)+
      geom_sf(data = ONBoundary,colour="black",fill=NA,lwd=0.3,show.legend = F) +
      ...
      annotate(geom="text",x=400,y=1650, label= "Residual OBBA2",lineheight = .85,hjust = 0,size=6,fontface =2) +
      #annotate(geom="text",x=400,y=1550, label= "OBBA 2 - Observed Data",lineheight = .85,hjust = 0,size=4,fontface =2) +
      annotate(geom="text",x=400,y=1480, label= paste0("Prepared on ",Sys.Date()),size=2,lineheight = .75,hjust = 0,color="gray75")+
      
      scale_color_gradientn(colors = colscale_resid,na.value=NA, name = "Residual", limits = col_lim)+
      scale_size_continuous(name = "# surveys", range = c(0,4), guide = "none")+
      scale_alpha_continuous(range = c(0,1), guide = "none")
    
    species_resid_OBBA2
    
    species_resid_OBBA3 <- ggplot() +
      geom_sf(data=ONBoundary,colour="gray90", fill = "white")+
      geom_sf(data=subset(species_hex_summary,Atlas == "OBBA3" & n>=10), aes(col = resid_mean, size = n))+
      geom_sf(data = range %>% st_intersection(ONBoundary), col = "gray50", fill = "transparent", linetype = 2)+
      geom_sf(data = ONBoundary,colour="black",fill=NA,lwd=0.3,show.legend = F) +
      ...
      annotate(geom="text",x=400,y=1650, label= "Residual OBBA3",lineheight = .85,hjust = 0,size=6,fontface =2) +
      #annotate(geom="text",x=400,y=1550, label= "OBBA 2 - Observed Data",lineheight = .85,hjust = 0,size=4,fontface =2) +
      annotate(geom="text",x=400,y=1480, label= paste0("Prepared on ",Sys.Date()),size=2,lineheight = .75,hjust = 0,color="gray75")+
    ...
    png(file = paste0("../Output/Prediction_Maps/Relative_Abundance/",species_name,"_",etype,".png"),
        units = "in", 
        width = 40, 
        height = 14, res = 600)
    
    grid.arrange(species_data_OBBA2,species_data_OBBA3, OBBA2_plot_q50,OBBA3_plot_q50,species_resid_OBBA2,species_resid_OBBA3,change_plot_q50,
                 ncol = 4,nrow=4, 
    ...
    species_change <- data.frame(species_name = species_name,
                                 ...
                                 error_type = etype,
                                 region = "ON",
                                 EDR = species_to_model$EDR[i],
                                 cue_rate = species_to_model$cue_rate[i],
                                 
                                 # Residual diagnostics
                                 Dispersion_OBBA2 = Dispersion_OBBA2$statistic,
                                 Dispersion_OBBA3 = Dispersion_OBBA3$statistic,
                                 ZeroInflation_OBBA2 = ZeroInflation_OBBA2$statistic,
                                 ZeroInflation_OBBA3 = ZeroInflation_OBBA3$statistic,
