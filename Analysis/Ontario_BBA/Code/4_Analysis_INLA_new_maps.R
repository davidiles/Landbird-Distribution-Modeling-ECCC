...
for (sp_code in rev(species_to_model$Species_Code_BSC)){
  ...
  #colpal_relabund <- colorRampPalette(colscale_relabund[1:(length(colscale_relabund)-3)])
  ...
  plot_q50 <- ggplot() +
    ...
    # Surveyed squares
    geom_sf(data = subset(SaskSquares_centroids, !is.na(PC_detected) | !is.na(CL_detected)), 
            aes(size = PC_mean_count),col = "gray50", pch = 1, alpha = 0.5)+
    
    # Point count detections
    geom_sf(data = subset(SaskSquares_centroids, !is.na(PC_detected) & PC_detected > 0), 
            aes(size = PC_mean_count),col = "black", pch = 1, alpha = 1)+
  ...
  plot_CI_width_90 <- ggplot() +
    ...
    scale_fill_manual(name = "<span style='font-size:13pt'>Uncertainty</span><br><span style='font-size:7pt'>Per 5-minute point count</span><br><span style='font-size:7pt'>Width of 90% CI</span>",
  ...
  raster_pObs = stars::st_rasterize(SaskGrid_species %>% dplyr::select(pObs_levs, x))
  ...
  # sum_BRT <- sum(SaskGrid_species$pred_BRT)
  # 
