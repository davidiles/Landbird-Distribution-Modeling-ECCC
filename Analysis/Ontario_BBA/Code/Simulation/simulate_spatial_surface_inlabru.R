...
# Fix these priors to a set value
# Controls the 'residual spatial field'.  This can be adjusted to create smoother surfaces.
matern_abund <- inla.spde2.pcmatern(mesh_spatial,
                                    prior.range = c(500,NA),#c(500, 0.01), # 500, NA # 1% chance range is smaller than 500000
                                    prior.sigma = c(0.1,NA),   # 10% chance sd is larger than 0.1,
                                    constr = TRUE
)
...
pc_prec <- list(prior = "pcprec", param = c(0.1, NA))
...
HSS_spde = inla.spde2.pcmatern(HSS_mesh1D,
                               prior.range = c(5,NA), # 10% chance range is smaller than 3
                               prior.sigma = c(2,NA)) # 10% chance sd is larger than 1) 
...
model_components = as.formula(paste0('~
            Intercept_OBBA2(1,model="linear",mean.linear = -10, prec.linear = 2e+10)+
            Intercept_OBBA3(1,model="linear", mean.linear = -10, prec.linear = 2e+10)+
...
pred <- generate(fit_INLA,
                 ONGrid_species,
                 formula =  
                   as.formula("~ data.frame(OBBA2 = Intercept_OBBA2 +log_offset_5min + spde_OBBA2,
                                     OBBA3 = Intercept_OBBA3 + log_offset_5min + spde_OBBA2 + spde_change)") ,
                 n.samples = n_samp,
                 seed = 123)
...
ONGrid_species$OBBA2_pred_q50 <- pred_OBBA2[,1]
ONGrid_species$OBBA3_pred_q50 <- pred_OBBA3[,1]
...
ONGrid_species$change_pred_q50 <-pred_change[,1]
