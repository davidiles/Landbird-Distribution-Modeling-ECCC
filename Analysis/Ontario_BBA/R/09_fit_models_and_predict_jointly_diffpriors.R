...
for (i in seq_len(nrow(species_run))) {
  ...
  prior_range_abund  <- c(500, 0.90) # 90% chance spatial autocorrelation range of shared field is less than 150 km
  prior_sigma_abund  <- c(3, 0.1)    # 10% chance SD is larger than 3
  prior_range_change <- c(500, 0.1)  # 10% chance spatial autocorrelation range of change field is less than 150 km
  ...
  if (file.exists(model_path) & rerun_models == FALSE) {
    ...
    mod <- try(
      ...
      fit_inla_multi_atlas(
        ...
        prior_sigma_change = c(0.1, 0.1),         # 10% chance SD is larger than 0.1
        kappa_pcprec_shared = c(0.25, 0.1),
        kappa_pcprec_diff   = c(0.25, 0.1),
        family = "poisson"),
