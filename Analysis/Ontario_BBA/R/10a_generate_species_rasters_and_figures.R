...
for (i in seq_len(length(pred_files))) {
  ...
  p2_dat <- make_relabund_map(
    ...
    survey_colour = "#E6C27A",
    detection_colour = "black",
    sp_hex_grid = sp_hex_grid %>% subset(Atlas == "OBBA2"),
    survey_circle_diameter_km = 5,
    max_detection_diameter_km = 4,
    min_detection_diameter_km = 0.75,
    survey_scaling_max = 10,
    alpha_min = 0.2,
    alpha_max = 1
  )
  
  ggsave(
    filename = file.path(fig_dir, paste0(sp_file, "_Atlas2_dat.png")),
    plot = p2_dat$sp_plot, width = 8, height = 10, units = "in",
    type = "cairo", 
    dpi = 1000,
    limitsize = FALSE
  )
  ...
  p3_dat <- make_relabund_map(
    ...
    survey_colour = "#E6C27A",
    detection_colour = "black",
    sp_hex_grid = sp_hex_grid %>% subset(Atlas == "OBBA3"),
    survey_circle_diameter_km = 5,
    max_detection_diameter_km = 4,
    min_detection_diameter_km = 0.75,
    survey_scaling_max = 10,
  ...
  ggsave(
    filename = file.path(fig_dir, paste0(sp_file, "_Atlas3_dat.png")),
    plot = p3_dat$sp_plot, width = 8, height = 10, units = "in",
    type = "cairo", 
    dpi = 1000,
    limitsize = FALSE
  )
