...
#' analysis. It:
#' - builds an SPDE mesh over the study boundary,
#' - defines abundance and change (difference) spatial fields,
#' - optionally includes 1D smoothers (Hours Since Sunrise; Day-of-Year),
#'   with BCR-specific deviations around a global smooth,
#' - includes an atlas-square random effect (shared and/or differential),
#' - fits a Poisson or Negative Binomial likelihood via `inlabru::bru()`,
#' - and returns the fitted model object plus key components needed for prediction.
#'
#' The function expects `sp_dat` to already contain standardized covariates,
#' derived time covariates, and indexing columns used by the model.
#'
#' @param sp_dat Survey data (`sf`) with required columns listed in the function.
#' @param study_boundary `sf` polygon boundary for mesh construction.
#' @param covariates Data frame defining fixed-effect covariates and priors.
#' @param timeout_min INLA timeout in minutes.
#' @param prior_range_abund,prior_sigma_abund PC priors for abundance SPDE range/sigma.
#' @param prior_range_change,prior_sigma_change PC priors for change SPDE range/sigma.
#' @param mesh_max_edge,mesh_cutoff,mesh_convex,mesh_concave Mesh construction controls.
#' @param HSS_prior_range,HSS_prior_sigma PC priors for the HSS 1D smoother.
#' @param DOY_prior_range,DOY_prior_sigma PC priors for the DOY 1D smoother.
#' @param kappa_pcprec_shared,kappa_pcprec_diff PC priors for square random effects.
#' @param family Likelihood family: "poisson" or "nbinomial".
...
fit_inla_multi_atlas <- function(sp_dat,
  ...
  components_str <- paste0(
    '~ Intercept(1) +
       ...
       DOY_global(main = days_rescaled, model = DOY_spde) +
       DOY_dev(main = days_rescaled, model = DOY_spde,
  ...
  #'
  #' Note: this function is defined twice in the current file; the second
  #' definition overwrites the first. If you keep both, ensure they are identical
  #' or rename one to avoid confusion.
  #'
...
#'
#' @param cov_df Optional covariate reference values (for marginal predictions).
...
# ============================================================
# Create a hexagon grid across study area, within which posterior samples can be summarized
# ============================================================
...
#'
#' @param mat Numeric matrix of posterior draws.
#' @param probs Quantiles to compute.
#' @return A data.frame of summaries with one row per prediction location.
...
#' @param newdata Data frame/sf of prediction locations and covariates.
#' @param formula Prediction formula/expression.
#' @param n_samples Number of posterior draws to generate.
#' @return A list containing draws and metadata suitable for summarization and mapping.
...
#'
#' @param sp_english Species common name.
#' @return Filename-safe character string.
...
#' @return Loaded object, or empty list.
...
#' Add derived covariates needed for modelling/prediction
#'
#' Ensures surveys and prediction grids contain consistent derived fields
#' (e.g., centered atlas indicator, BCR indices, atlas-square indices, and
#' transformed covariates). This is often called before model fitting so
#' required columns exist with the expected meaning.
#'
#' @param surveys Survey `sf` with extracted covariates.
#' @param grid2 Prediction grid for Atlas 2.
#' @param grid3 Prediction grid for Atlas 3.
#' @param covars_to_standardize Character vector of covariates to standardize.
#' @param covars_for_stats Character vector used to compute scaling stats.
#' @return A list with updated surveys/grid2/grid3 objects.
add_derived_covariates <- function(surveys, grid2, grid3,
                                   south_bcr = c(12, 13), north_bcr = c(7, 8)) {
  
  #' Create indexing bits (integer IDs) for categorical effects
  #'
  #' Internal helper used by `add_derived_covariates()` to convert factors like
  #' BCR and atlas-square into compact integer indices required by INLA random
  #' effect terms.
  #'
  #' @param df Data frame/sf containing categorical columns.
  #' @return A modified data frame with added *_idx columns.
