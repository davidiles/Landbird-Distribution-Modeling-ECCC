...
obba3_path <- file.path(paths$data, "Bird_Data_Raw","OBBA3","Checklist","onatlas3be_do_naturecounts_data.txt")
NC_OBBA3_CL_raw <- read.table(
  obba3_path,
  sep = "\t", header = TRUE, fill = TRUE, comment.char = "", quote = ""
) %>%
  select(where(~ sum(!is.na(.x)) > 0)) %>%
  ...
  mutate(
    ...
    TimeObservationsStarted = as.numeric(TimeObservationsStarted),
    DurationInHours = as.numeric(DurationInHours),
  ...
  # Removed mislabeled effort units
...
NC_OBBA3_CL <- NC_OBBA3_CL_raw %>%
  select(
    ...
    DecimalLatitude, DecimalLongitude,
    Date_Time, DurationInHours,Distance_Traveled_m,
    species_id, ObservationCount
  )

# Only include checklist surveys between 1 and 10 minutes in duration, and less than 1 km distance
n1 = nrow(NC_OBBA3_CL)
NC_OBBA3_CL <- NC_OBBA3_CL %>%
  na.omit() %>%
  
  subset(
    
    # This removes about 90% of records
    DurationInHours >= (1/60) & DurationInHours <= (10/60) &         
         Survey_Type %in% c("Breeding Bird Atlas","Linear transect") &
      
    # Removes a further 2% of records
    Distance_Traveled_m <= 500) 

# Remove linear transect checklists with no distance
NC_OBBA3_CL <- NC_OBBA3_CL %>%
  subset(Survey_Type == "Breeding Bird Atlas" |
         (Survey_Type == "Linear transect" & !is.na(Distance_Traveled_m)))
