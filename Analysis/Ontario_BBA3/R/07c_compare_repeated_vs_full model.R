...
for (i in seq_along(pred_files)) {
  ...
  full_data_summary <- sp_dat %>%
    as.data.frame() %>%
    group_by(Biol_Region,Atlas, sq_) %>%
    summarize(mean_count = mean(count),
              PObs = mean(count>0),
              n_surveys = n(),
              n_squares = length(unique(square_id))) %>%
    left_join(.,detection_summary) %>%
...
calc_raw_change <- function(data_summary,
                            value_cols = c("mean_count", "PObs")) {
  
  extra_cols <- intersect(
    c("n_sq_detected_full"),
    names(data_summary)
  )
  ...
  data_summary %>%
    ...
    dplyr::mutate(
      ...
      raw_pct_change_PObs =
        100 * (PObs_OBBA3 - PObs_OBBA2) / PObs_OBBA2,
      
      raw_log_change_PObs =
        log(PObs_OBBA3 / PObs_OBBA2),
      
      delta_n_surveys =
        n_surveys_OBBA3 - n_surveys_OBBA2,
...
diagnostic_comparison <- full_model_change %>%
  ...
  dplyr::mutate(
    ...
    raw_PObs_difference =
      full_raw_pct_change_PObs - repeated_raw_pct_change_PObs,
    ...
    min_repeated_surveys =
      pmin(repeated_n_surveys_OBBA2, repeated_n_surveys_OBBA3, na.rm = TRUE),
    ...
    min_full_detected_squares =
      pmin(
        full_n_sq_detected_full_OBBA2,
        full_n_sq_detected_full_OBBA3,
        na.rm = TRUE
      )
  )
...
diagnostic_comparison_filtered <- diagnostic_comparison %>%
  dplyr::filter(
    min_full_detected_squares >= 10,
    
    repeated_n_squares_OBBA2 >= 10,
    repeated_n_squares_OBBA3 >= 10,
    
    repeated_n_surveys_OBBA2 >= 50,
    repeated_n_surveys_OBBA3 >= 50,
    
    full_mean_count_OBBA2 > 0.005,
    repeated_mean_count_OBBA2 > 0.005,
    
    is.finite(raw_data_difference),
    is.finite(raw_PObs_difference),
    is.finite(model_difference),
    is.finite(full_model_pct_change),
    is.finite(repeated_model_pct_change)
  )
...
for (RN in diagnostic_comparison_filtered$Region_Name){
