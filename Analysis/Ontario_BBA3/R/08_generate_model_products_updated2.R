...
for (i in seq_along(pred_files)) {
  ...
  # Relative-abundance rasters are thresholded using a hybrid approach
  # intended to suppress biologically negligible predictions while adapting
  # to differences in abundance among species.
  #
  # Pixels are treated as effectively absent if predicted abundance falls
  # below the largest of:
  #
  #   1. A fixed minimum abundance threshold
  #      (e.g., 1 expected individual per 500 point counts),
  #
  #   2. A low species-specific abundance quantile
  #      (e.g., the bottom 1% of predictions),
  #
  #   3. A small fraction of the species' typical high-abundance values
  #      (e.g., 1% of the 99th percentile prediction).
  #
  # This approach reduces diffuse low-abundance "background haze" for common
  # species while avoiding overly restrictive thresholds for rare species.
  # Consequently, mapped presence should be interpreted as areas of
  # biologically meaningful predicted occurrence rather than strict
  # occupancy/absence.
  
  # Assume the species is effectively absent if model predicts expected count less than 1 per 500 point counts
  ...
  paired_change_summary <- paired_summaries[[sp_english]]$shared_change_summary %>%
    dplyr::rename(pct_change_median = pct_change_q50,
                  pct_change_qlow = pct_change_q05,
                  pct_change_qhigh = pct_change_q95)
  paired_data <- paired_summaries[[sp_english]]$shared_data
  
  # Hide change estimates if species was not detected in more than 10 squares in any atlas
  min_sq_det <- 10
  
  data_availability <- sp_dat$sp_dat %>%
    as.data.frame() %>%
    dplyr::filter(count > 0) %>%
    dplyr::group_by(Atlas, Biol_Region) %>%
    dplyr::summarise(
      n_sq_det = dplyr::n_distinct(square_id),
      .groups = "drop"
    ) %>%
    tidyr::pivot_wider(
      names_from = Atlas,
      values_from = n_sq_det,
      names_prefix = "n_sq_det_",
      values_fill = 0
    ) %>%
    dplyr::mutate(
      sufficient_data =
        n_sq_det_OBBA2 >= min_sq_det |
        n_sq_det_OBBA3 >= min_sq_det
    )

  # Mask out insufficient data regions
  ...
  regional_estimates_FullModel_masked <- regional_estimates_FullModel %>%
    dplyr::left_join(
      data_availability,
      by = c("Region_Number" = "Biol_Region")
    ) %>%
    dplyr::mutate(
