...
#
# Purpose
#   Create a regular 1-km prediction grid over the (buffered) study area, in
#   EPSG:3978 to align with the processed rasters. Produces grid-cell polygons
#   (for area-based covariate extraction) and grid centroids (prediction points).
#
# Output
#   data_clean/spatial/prediction_grid_<cellsize>_m.rds
# ============================================================
...
# Use the buffered boundary for the prediction domain (trimmed to the exact
# boundary later, in 05).
boundary <- study_area$boundary_buffer_5km %>%
  st_transform(grid_crs) %>%
  st_make_valid()

# ------------------------------------------------------------
# Build grid polygons and centroids
# ------------------------------------------------------------

grid_polys <- st_make_grid(
  boundary, cellsize = grid_cellsize_m, what = "polygons", square = TRUE
) %>%
  st_as_sf() %>%
  rename(geometry = x)

# Centroids are safe here: the grid is projected and cells are regular squares.
grid_pts <- st_centroid(grid_polys)

# Keep cells whose centroid falls inside the boundary.
