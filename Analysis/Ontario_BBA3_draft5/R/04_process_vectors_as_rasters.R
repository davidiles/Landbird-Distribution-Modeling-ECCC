...
# =============================================================================
# Load, filter, buffer, and rasterize road layers
#
# Winter-road filtering:
#   In northern Ontario (>50 degrees N), roads whose NAME contains the
#   standalone word "Winter" are assumed to be seasonal winter roads and
#   are excluded from the road covariate.
#
#   The whole-word regex "\\bwinter\\b" ensures that names such as
#   "Wintergreen" or "Winterbury" are NOT classified as winter roads.
#
#   South of 50 degrees N, roads are retained regardless of whether their
#   name contains "Winter", because these may be permanent roads that simply
#   have "Winter" as part of their proper name.
#
#   Latitude is determined from the centroid of each road feature after
#   temporarily transforming its geometry to geographic coordinates
#   (EPSG:4326). The road geometry itself remains in study_crs.
# =============================================================================
...
# Calculate the approximate latitude of each road feature.
# Transform a copy of the geometry to lon/lat (EPSG:4326), calculate its
# centroid, and extract the Y coordinate (= latitude).
roads_2005$.latitude <- roads_2005 %>%
  sf::st_geometry() %>%
...
# Remove roads that:
#   1. are north of 50 degrees N, AND
#   2. contain the standalone word "winter" in NAME (case-insensitive).
#
# coalesce(..., FALSE) ensures that roads with NAME = NA are retained.
roads_2005 <- roads_2005 %>%
  dplyr::filter(
    !(
      .latitude > 50 &
        dplyr::coalesce(
          stringr::str_detect(
            NAME,
            stringr::regex("\\bwinter\\b", ignore_case = TRUE)
          ),
          FALSE
        )
    )
...
# Buffer roads by 125 m (0.125 km), rasterize, and save.
buf_rasterize_save(
  roads_2005,
  buffer_dist_km = 0.125,
  out_path = file.path(
    out_spatial_dir,
    "roads_2005_buf_125m.tif"
  ),
  label = "roads_2005"
)

# Remove the large sf object from memory before processing the next layer.
rm(roads_2005)
gc()
...
# Remove winter roads (class 87)
roads_2025 <- roads_2025 %>%
  filter(CLASS != 87)

# Calculate the approximate latitude of each road feature.
roads_2025$.latitude <- roads_2025 %>%
  sf::st_geometry() %>%
  sf::st_transform(4326) %>%
  sf::st_centroid() %>%
  sf::st_coordinates() %>%
...
# Remove named winter roads north of 50 degrees N.
roads_2025 <- roads_2025 %>%
  dplyr::filter(
    !(
      .latitude > 50 &
...
# ------------------------------------------------------------
# Hudson Bay coastline (buffer = 3 km)
# ------------------------------------------------------------
...
# 4. Final 3 km coastal covariate from the boundary-derived coastline.
buf_rasterize_save(
  coastline_boundary, buffer_dist_km = 3,
  out_path = file.path(out_spatial_dir, "coastline_buf_3km.tif"),
  label = "coastline"
)
...
# ------------------------------------------------------------
# Rivers
#   large rivers : waterbody polygons (River/Canal), buffered 500 m
#   small rivers : watercourse lines (Stream),       buffered 250 m,
#                  then large-river cells zeroed out
# ------------------------------------------------------------

message("\n-- Rivers -------------------------------------------------")

# River waterbody polygons must be larger than 5000 m2 in area (removes tiny and misclassified waterbody fragments)
large_rivers <- waterbody %>%
  dplyr::filter(WATERBODY_ %in% c("River", "Canal"), PERMANENCY == "Permanent") %>%
  mutate(area = sf::st_area(geometry) %>% as.numeric()) %>%
  filter(area >= 0.1) %>%
  sf::st_make_valid()

large_rivers_r <- buf_rasterize_save(
  large_rivers, buffer_dist_km = 0.5,
  out_path = file.path(out_spatial_dir, "rivers_large_buf_500m.tif"),
  label = "large_rivers"
)
...
small_rivers_r <- buf_rasterize_save(
  small_rivers, buffer_dist_km = 0.25,
...
terra::writeRaster(small_rivers_r,
                   file.path(out_spatial_dir, "rivers_small_buf_250m.tif"),
...
buf_rasterize_save(
  great_lakes_boundary, buffer_dist_km = 3,
  out_path = file.path(out_spatial_dir, "great_lakes_buf_3km.tif"),
  label = "great_lakes"
)
...
# ------------------------------------------------------------
# Non-GL lakes: small (< 1 km2) and large (>= 1 km2).  Large lakes are buffered to 500 m, small lakes buffered to 250m
# ------------------------------------------------------------
...
buf_rasterize_save(large_lakes, buffer_dist_km = 0.500,
                   out_path = file.path(out_spatial_dir, "lakes_large_buf_500m.tif"),
                   label = "large_lakes")
