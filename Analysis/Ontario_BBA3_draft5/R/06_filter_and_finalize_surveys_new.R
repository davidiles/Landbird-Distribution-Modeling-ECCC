...
add_derived_covariates <- function(df,
  ...
  df %>%
    mutate(
      ...
      Wetland_North = if_else(BCR %in% c("76", "77", "74"),   get_or_zero(wetland_var), 0),
      Wetland_South = if_else(!(BCR %in% c("76", "77", "74")), get_or_zero(wetland_var), 0),
...
MAX_TRANSECT_M      <- 1000   # upper bound on transect length
...
MAX_DURATION_MIN    <- 60    # general upper bound
...
# NA-duration surveys (~30k, almost all checklists) are omitted: without a
# duration they cannot receive the effort/duration handling the model relies on.
# Count and report them here so the omission is explicit in every run rather than
# happening silently inside the filter below. (EffortUnits* investigation into
# whether duration lives elsewhere for these rows is still open.)
n_na_duration <- sum(is.na(all_surveys$Survey_Duration_Minutes))
message("Omitting ", n_na_duration,
        " surveys with NA Survey_Duration_Minutes (almost all checklists).")
...
surveys_f <- all_surveys %>%
  ...
  filter(
    ...
    # Omit NA-duration surveys explicitly (counted and reported above), then keep
...
for (sp_id in species_ids) {
  ...
  if (file.exists(sp_path) && !rebuild_species_data) {
    ...
    if (save_species_shapefile){
      sp_shapefile_path <- file.path(species_dir, paste0(sp_filename(sp_name), "_data.shp"))
      
      sp_shapefile <- surveys_f %>%
        dplyr::select(survey_id, square_id, Survey_Type, Date_Time_Local,
                      Survey_Duration_Minutes, Distance_Traveled_m) %>%
        dplyr::mutate(survey_id = as.character(survey_id)) %>%
        dplyr::inner_join(
          sp_record$survey_counts %>% dplyr::mutate(survey_id = as.character(survey_id)),
          by = "survey_id"
        ) %>%
        filter(count>0)
      
      sf::st_write(sp_shapefile, dsn = sp_shapefile_path,delete_layer = TRUE)
    }
  }
