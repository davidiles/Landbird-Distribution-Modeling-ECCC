...
# Squares-with-detection threshold that defines a "sufficiently observed" BCR.
# A BCR is sufficient if the species was detected in >= min_sq_det squares in
# EITHER atlas. This single threshold governs BOTH:
#   (1) the province-wide aggregate -- only sufficient BCRs contribute to the
#       province sum. Insufficient BCRs (including the many with zero detections)
#       are dropped, because the model's exp(spatial field) can extrapolate
#       large, highly variable abundance into unsampled regions and hijack the
#       province total. "Either atlas" is deliberate: a region occupied in only
#       one period (real colonisation/extirpation) is retained, not trimmed.
#   (2) the optional BCR-row masking below (MASK_INSUFFICIENT) -- insufficient
#       BCR rows have their change columns NA'd out.
# Province rows are never masked; the province sum is only restricted, never
# blanked. NOTE: this one value controls both range-restriction and masking,
# so set it deliberately -- a higher value restricts the province footprint more.
...
# Union the area-weight tables of a set of BCRs into one province-subset weight
# table (hex_id -> summed area_weight). The BCRs partition the study area, so a
# hex straddling several retained BCRs recovers its total retained fraction; the
# pmin() is a defensive cap against tiny overlap slivers.
combine_region_weights <- function(region_weights, bcr_keep) {
  dplyr::bind_rows(region_weights[as.character(bcr_keep)]) %>%
    dplyr::group_by(hex_id) %>%
    dplyr::summarise(area_weight = sum(area_weight), .groups = "drop") %>%
    dplyr::mutate(area_weight = pmin(area_weight, 1))
}
...
summarize_species_regions <- function(hex_draws, ci_level, retain_bcrs = NULL) {
  
  # Province aggregate. Default: sum every hex in the study boundary. When
  # retain_bcrs is supplied, sum over ONLY those BCRs' hexes instead.
  province_trimmed <- !is.null(retain_bcrs)
  prov_weights <- if (province_trimmed) {
    combine_region_weights(region_weights, retain_bcrs)
  } else {
    region_weights[["province"]]
  }
  
  prov <- summarize_polygon_hex_draw_change(
    hex_draws   = hex_draws,
    hex_grid    = hex_grid,
    hex_weights = prov_weights,
    ci_level    = ci_level
  ) %>%
    dplyr::mutate(
      ...
      province_trimmed   = province_trimmed,
      province_bcrs_kept = if (province_trimmed)
        paste(sort(retain_bcrs), collapse = ",") else NA_character_,
      .before = 1
    )
...
for (i in seq_along(dat_used_files)) {
  ...
  # ---- Detection counts + sufficiency (computed BEFORE the province aggregate
  #      so the SAME sufficiency rule can both mask BCR rows and restrict the
  #      province sum) ----
  sp_surveys <- load_species_surveys(dat_used, all_surveys)
  avail      <- species_detection_availability(sp_surveys)
  
  # Harden against a species detected in only one atlas (pivot would omit the
  # other column); also protects the coalesce() below.
  for (cc in c("n_sq_det_OBBA2", "n_sq_det_OBBA3"))
    if (!cc %in% names(avail)) avail[[cc]] <- 0L
  
  # Sufficient = detected in >= min_sq_det squares in EITHER atlas. Defined once
  # here and reused for both the province restriction and the BCR-row mask.
  avail <- avail %>%
    dplyr::mutate(
      sufficient_data = n_sq_det_OBBA2 >= min_sq_det | n_sq_det_OBBA3 >= min_sq_det
    )
  
  # BCRs that contribute to the province sum. avail carries only BCRs with sufficient
  # detections, so zero-detection BCRs are excluded implicitly.
  retain_bcrs <- avail %>%
    dplyr::filter(Region_Number %in% bcr_regions$BCR, sufficient_data) %>%
    dplyr::pull(Region_Number)
  
  if (length(retain_bcrs) == 0) {
    message("  ", sp_english, ": no BCR clears min_sq_det = ", min_sq_det,
            "; province total left UNTRIMMED (province_trimmed = FALSE).")
    retain_bcrs <- NULL   # fall back to full-province aggregate, but flagged
  }
  
  # ---- Core: province (range-restricted) + per-BCR posterior change ----
  est <- summarize_species_regions(hex_draws, ci_level = ci_level,
                                   retain_bcrs = retain_bcrs) %>%
    dplyr::mutate(
  ...
  # ---- Join detection counts + sufficiency flag (both already on avail) ----
  # BCRs/province absent from avail (no detections) coalesce to 0 / FALSE.
  est <- est %>%
    ...
    dplyr::mutate(
      ...
      sufficient_data = dplyr::coalesce(sufficient_data, FALSE)
    )
