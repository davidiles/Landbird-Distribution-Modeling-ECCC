...
source(file.path(paths$functions, "model_product_utils_SS.R"))
source(file.path(paths$functions, "inla_model_utils_SS.R"))   # shared helpers
...
cfg <- list(
  ...
  # which the low-density tail contains at most 1% of the predicted population.
  # Then CAP that cutoff at 1/1000 expected birds per standardized 5-min count.
  # Thus the final threshold is min(T_99.5%, 1/1000): the 99.5% rule can make the map
  # more inclusive, but can never mask values above 1/1000.
  relabund_coverage = 0.99,
...
i = which(dat_used_files == "C:/Users/IlesD/OneDrive - EC-EC/Iles/Projects/Landbirds/Landbird-Distribution-Modeling-ECCC/Analysis/Ontario_BBA3_draft5/data_clean/model_output/data_used_PC_ARU_CL/red_breasted_merganser_1km.rds")
...
for (i in seq_along(dat_used_files)) {
  ...
  if (!has_model) {
    ...
    A2_special <- honey_prep$special %>% dplyr::filter(Atlas == "OBBA2")
    A3_special <- honey_prep$special %>% dplyr::filter(Atlas == "OBBA3")
    ...
    honey_pages <- lapply(region_specs, function(spec) {
      h <- build_period_honeycombs(
        ...
        A2_dat           = A2_obs,
        A3_dat           = A3_obs,
        A2_special       = A2_special,
        A3_special       = A3_special,
  ...
  rasters_relabund_prepared <- prepare_relative_abundance_rasters(
    ...
    max_absent_limit  = cfg$relabund_absent_limit,  # cap: final threshold = min(T_99%, 1/500)
