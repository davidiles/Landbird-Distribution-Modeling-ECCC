...
detection_counts_model <- model_A
...
mean(plot_df$direction_agree,na.rm = TRUE)


plot_df %>%
  group_by(region_key) %>%
  summarize(n_species = n(),
            prop_direction_agree = mean(direction_agree,na.rm = TRUE),
            prop_PC_ARU_higher = mean(est_A > est_B,na.rm = TRUE))

subset(plot_df, agreement == "Disagree" & est_B <= -30 & est_A >= 0 & region_key == "BCR13") %>% as.data.frame()
