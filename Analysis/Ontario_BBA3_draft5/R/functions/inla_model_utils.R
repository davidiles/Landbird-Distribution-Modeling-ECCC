...
fit_PC_ARU_CL <- function(
    ...
    # Error families (NO fallback -- a failure is raised). The STATIONARY
    # observation model (PC + ARU + BBA) shares ONE family and, when nbinomial,
    # ONE size; linear transects (LT) get their own. Two sizes total, not three,
    # which are far easier to identify. Only the ERROR structure differs by
    # stream -- the site-level random effect is shared (3b).
    family_stationary = c("nbinomial", "poisson"),
    family_lt         = c("nbinomial", "poisson"),
    
    # Negative-binomial PC prior settings (stationary model).
    nb_pc_target_prob     = 0.5,
    nb_pc_threshold_theta = 5,
    
    # Negative-binomial PC prior settings (linear transects). Default to the
    # stationary values, so the LT size shares the SAME prior but is a SEPARATE
    # parameter (own likelihood below). Set independently to give the heavier,
    # effort-driven LT overdispersion its own prior.
    nb_cl_target_prob     = nb_pc_target_prob,
    nb_cl_threshold_theta = nb_pc_threshold_theta,
  ...
  family_stationary <- match.arg(family_stationary)
  family_lt         <- match.arg(family_lt)
  ...
  # is far better identified than two. Error structures still differ by stream
  # (family_stationary vs family_lt); the site-level correlation does not.
  #
  # Computed HERE, not upstream: sp_dat has already been through species-specific
  ...
  message(
    "  likelihoods: ", n_pc_aru + n_bba, " stationary (",
    n_pc_aru, " PC/ARU + ", n_bba, " BBA)",
    if (n_lt > 0) paste0(" + ", n_lt, " LT") else "",
    " (separate sizes where nbinomial)"
  )
  ...
  components_str <- paste0(
    ...
    # BBA (stationary checklist) intercept + duration-effort components, defined
    # ONLY when BBA rows exist -- i.e. iff those terms enter a predictor below --
    # so no component is ever left referenced by zero likelihoods.
    if (n_bba > 0) {
      " + effect_SC(
             1,
             model = 'linear',
    ...
    # LT (linear transect) intercept + distance-effort components, defined ONLY
    # when LT rows exist (iff the LT likelihood is built below).
    if (n_lt > 0) {
      " + effect_LT(
             1,
             model = 'linear',
  ...
  # Terms shared by BOTH observation models: the latent abundance/change surface,
  # shared detectability (TOD, DOY), the square and SITE random effects, and the
  # covariates. The site term lives here -- not in a per-stream predictor --
  # which is what makes LT and stationary rows share one site effect.
  shared_terms <- paste0(
    "count ~
      Intercept +
      TOD_global +
  ...
  # Per-likelihood predictors. Each carries ONLY the protocol/effort terms whose
  # weights are non-zero on its own rows, so inlabru builds no zero-weight
  # projectors for the other protocol's components. Those terms contributed
  # exactly zero under a fully shared formula, so this is the SAME model at lower
  # linearisation cost.
  ...
  formula_stationary <- stats::as.formula(paste0(
    shared_terms,
    " +
      ARU_1min * effect_ARU_1min +
      ARU_3min * effect_ARU_3min +
  ...
  formula_lt <- stats::as.formula(paste0(
    shared_terms,
    " +
      LT  * effect_LT +
  ...
  # ------------------------------------------------------------
  # 12. Build likelihood(s)
  # ------------------------------------------------------------
  # Two observation models over the SAME components: a STATIONARY model
  # (PC + ARU + BBA, one shared size) and a LINEAR-TRANSECT model (own size),
  # each with its OWN predictor carrying only the terms active on its rows.
  # Terms omitted from a predictor are zero on those rows, so the mean structure
  # is unchanged. The families may differ; the site effect does not.
  
  build_lik <- function(rows, fam, formula, target_prob, threshold_theta) {
    if (fam == "poisson") {
      inlabru::like(
        family  = "poisson",
        formula = formula,
  ...
  # Stationary (PC + ARU + BBA): one likelihood, one size. Always present.
  liks      <- list(build_lik(
    rows            = is_stationary,
    fam             = family_stationary,
    formula         = formula_stationary,
    target_prob     = nb_pc_target_prob,
    threshold_theta = nb_pc_threshold_theta
  ))
  lik_order <- "stationary"
  
  # Linear transects (LT): own likelihood / own size, only when present.
  if (n_lt > 0) {
    liks      <- c(liks, list(build_lik(
      rows            = is_lt,
      fam             = family_lt,
      formula         = formula_lt,
      target_prob     = nb_cl_target_prob,
      threshold_theta = nb_cl_threshold_theta
    )))
    lik_order <- c(lik_order, "LT")
  }
  ...
  while (attempt <= retry) {
    ...
    # do.call so the 1-vs-2 likelihood case is handled uniformly: components
    # first (positional), each likelihood into `...`, options by name.
    fit <- try(
      do.call(
        inlabru::bru,
        c(list(model_components), liks, list(options = bru_opts))
      ),
      silent = TRUE
    )
  ...
  # Which observation model is which, so the nbinomial size rows in
  # summary.hyperpar can be disambiguated downstream. Order matches `liks`:
  # size[k] -> lik_order[k].
  fit$lik_order         <- lik_order
  fit$family_stationary <- family_stationary
  fit$family_lt         <- family_lt
