...
for (sp_code in species_to_model$Species_Code_BSC){
  ...
  model_file <- paste0("../Output/Fitted_Models/Model_",sp_code,"_integrated.rds")
  ...
  sp_dat <- all_surveys %>% 
    ...
    subset(Survey_Type %in% c("Point_Count",
                              "ARU_SPT",
                              "ARU_SPM",
                              "Breeding Bird Atlas",
                              "Linear transect"))
  ...
  print(paste0(sp_code," - ",runtime_integrated)) # ~10 min 
  
  species_fit <- list(sp_dat = sp_dat,
                      fit_integrated = fit_integrated)
