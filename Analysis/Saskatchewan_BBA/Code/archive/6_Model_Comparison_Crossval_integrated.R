...
covars_for_PCA <- all_surveys %>%
  as.data.frame() %>%
  dplyr::select(elevation_1km:Water_10km)
...
for (xval_fold in 1:n_folds){
  for (sp_code in species_for_crossval$Species_Code_BSC){
    ...
    matern_coarse <- inla.spde2.pcmatern(mesh_spatial,
                                         ...
                                         prior.sigma = c(1, 0.1), # 10% chance sd is larger than 2
    ...
    LT_duration_meshpoints <- seq(min(training_data_LT$Survey_Duration_Minutes)/1.2,max(training_data_LT$Survey_Duration_Minutes)*1.2,length.out = 11)
    ...
    start <- Sys.time()
    fit_training_PC <- bru(components = model_components,
    ...
    start <- Sys.time()
    fit_training_PC_SC <- bru(components = model_components,
    ...
    start <- Sys.time()
    fit_training_PC_LT <- bru(components = model_components,
    ...
    fit_training_BRT <- fit_brt(model_data =  training_data_PC_df,
